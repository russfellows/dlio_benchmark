"""
CSV reader using parallel prefetch from S3-compatible object storage.
...
object storage. Parsing CSV rows is pure CPU overhead that adds noise to the
...
NPYReaderS3Iterable and NPZReaderS3Iterable. No pandas, no polars, no parsing.
...
class CSVReaderS3Iterable(CSVReader, _S3IterableMixin):
    """
    Parallel-prefetch CSV reader for S3-compatible object stores.

    Fetches every assigned object in parallel and stores only the raw byte
    count (int) — no CSV parsing, no pandas, no polars. get_sample() reports
    that byte count as the image_size telemetry metric. The actual I/O transfer
    is fully measured; the omitted parse step is pure CPU overhead irrelevant
    to storage benchmarking.
    ...
    def __init__(self, dataset_type, thread_index, epoch):
        ...
        self._s3_init(opts)
        self.logger.info(
            f"{utcnow()} CSVReaderS3Iterable [{self._storage_library}] "
    ...
    def get_sample(self, filename, sample_index):
        # Report byte count for telemetry. Do NOT call super() — CSVReader.get_sample()
        # tries to index open_file_map[filename] as a numpy array, which would fail
